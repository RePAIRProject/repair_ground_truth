...
solved_puzzle_folder = os.path.join(exported_OBJ_folder, 'SOLVED')
json_folder = os.path.join(exported_OBJ_folder, 'JSON')
os.makedirs(exported_OBJ_folder, exist_ok=True)
os.makedirs(solved_puzzle_folder, exist_ok=True)
...
for solved_puzzle in list_of_solved_puzzles:
    ...
    if group_num > 47: #1 > 0: #solved_puzzle == 'group_82_DONE_new.blend':
        ...
        for collection in collections:
            ...
            if "O" in collection.name or "Collection" in collection.name \
                ...
                if len(collection.all_objects) > 0:
                    ...
                    solved_pieces_folder = os.path.join(solved_puzzle_folder, puzzle_name)
                    os.makedirs(solved_pieces_folder, exist_ok=True)
                    json_name_v1 = f"RPobj_g{group_num}_o{puzzle_counter:04d}.json"
                    target_gt_path = os.path.join(json_folder, json_name_v1)
